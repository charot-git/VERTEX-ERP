...
public class CustomerCreditMemoLoader implements MemoDataLoader {
    CustomerMemoDAO customerMemoDAO = new CustomerMemoDAO();
    @Override
    public void loadMemoData(TableView<CreditDebitMemo> memoTable) {
        ObservableList<CreditDebitMemo> memoList = FXCollections.observableList(customerMemoDAO.getAllCustomerCreditMemo());
        if (memoList.isEmpty()) {
