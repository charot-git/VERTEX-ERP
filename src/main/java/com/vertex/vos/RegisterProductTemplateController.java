...
public class RegisterProductTemplateController implements Initializable, DateSelectedCallback {
    ...
    private static final String UPDATE_PRODUCT_QUERY = "UPDATE products SET " +
            "product_name=?, product_code=?, price_per_unit=?, cost_per_unit=?, description=?, " +
            "product_discount=?, quantity_available=?, date_added=?, supplier_name=?, " +
            "priceA=?, priceB=?, priceC=? WHERE product_id=?";
    ...
    private Product selectedProduct; // Hold the selected product for editing
    ...
    private boolean updateProduct() {
        try (Connection connection = dataSource.getConnection();
            ...
            preparedStatement.setDate(8, java.sql.Date.valueOf(dateAddedTextField.getText()));
            preparedStatement.setString(9, supplierNameComboBox.getSelectionModel().getSelectedItem());
            preparedStatement.setDouble(10, Double.parseDouble(priceATextField.getText()));
            preparedStatement.setDouble(11, Double.parseDouble(priceBTextField.getText()));
            preparedStatement.setDouble(12, Double.parseDouble(priceCTextField.getText()));
            preparedStatement.setInt(13, selectedProduct.getProductId()); // Assuming selectedProduct.getProductId() retrieves the correct product_id
